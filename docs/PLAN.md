...
当前版本：v0.9.1（2026-09-04）｜技术栈：Python 3 标准库（http.server / sqlite3），零新依赖

> **实施计划**：v0.4 → v0.7 的详细实施（决策表 D1-D10、22 个 Task、TDD 步骤与完整代码）见
> [`docs/PLAN-v0.4-v0.7.md`](docs/PLAN-v0.4-v0.7.md)。
> 里程碑进度：✅ v0.4 统计基础 → ✅ v0.5 交叉分析 → ✅ v0.6 对比与维度 → ✅ v0.7 交付加固（xlsx/token/分组）→ ✅ v0.8 编辑器与预览体验 → ✅ v0.9 版本基线（无新功能）→ ✅ v0.9.1 导航与报表浏览重构。
...
18. ✅ 编辑器参数抽屉化（紧凑列表 + 右滑配置面板，JS 模型单数据源）+ 只读预览页 `/pv/{id}` + 数值原样展示（移除千分位）+ 驱动懒加载（缺驱动中文提示，start.bat 引导安装）✅ v0.8
...
✅ v0.4 统计基础 → ✅ v0.5 交叉分析 → ✅ v0.6 对比与维度 → ✅ v0.7 交付加固（xlsx / token / 分组，Task 19-22 全部落地）→ ✅ v0.8 编辑器与预览体验 → ✅ v0.9 版本基线 → v1.0 对外交付（P0 真库连通 / P2-4 部署 systemd+Docker / CHANGELOG 定版）。
