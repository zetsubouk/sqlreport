...
- **目录结构标准化**：`PLAN.md` / `DEVLOG.md` / `IDEA.md` 归档至 `docs/`；删除根级兼容 shim
  （`server.py` / `db.py` / `params.py` / `xlsx.py` / `analytics.py`），`src/sqlreport/` 为唯一可信源码；
  运行方式统一为 `python -m sqlreport` / `sqlreport`；测试改用 `from sqlreport.…` 导入
- **跨平台启停脚本**：新增 `scripts/start.sh` / `scripts/stop.sh`（macOS/Linux 通用，POSIX sh），
  自检 Python 3.10+、自动创建 `.venv` 并 `pip install -e .`，缺 `pymysql` / `pyodbc` 时交互式可选安装，
...
- **导航重构**：导航栏调整为「报表列表 / 浏览报表 / 数据源管理」，取消「新建报表」独立导航项
  （`/new` 路由保留，入口移至列表页右上角按钮）
- **报表列表收敛**：列表页（`/`）只显示未分组报表主表，移除各分组折叠列表；
  分组报表统一在「浏览报表」页按分组展示
- **新增「浏览报表」（`/browse`）**：只读目录页——未分组报表归入「默认分组」，各分组独立一块，
  每张报表仅「打开」（新窗口进入查看页）；默认分组无报表时整块隐藏；
  全页无任何新建/编辑/删除入口
- **编辑器保存拆分**：原「保存并生成URL」拆为【保存】（仅保存原地不动，新建保存后原地变编辑态）
  与【保存并预览】（保存后新窗口打开查看页）
- **查看页导航按钮**：`/r/<id>` 右上角新增【返回】（回报表列表）与【编辑】（进入编辑页），
  移除面包屑；轻量预览页 `/pv/<id>` 保持纯只读不受影响

### 兼容性

- 报表 JSON 格式、独立 URL、查询/导出链路与 v0.9.0 完全一致，旧报表零迁移；
  仅页面导航布局变化
...
- **版本号递增**：`0.8.0` → `0.9.0`（`src/sqlreport/__init__.py` 与 `pyproject.toml` 同步）；本版不引入新功能，为后续开发提供独立版本基线

### 兼容性

- 对外路由、报表 JSON 格式与旧版完全一致，旧报表零迁移

## \[v0.8.0] - 2026-09-03

### 新增

- **编辑器参数配置抽屉化**：查询参数区从「每参数全量配置平铺」改为紧凑按序列表（每行 = 类型标签 + 名称 + 参数ID + 配置 / 上移 / 下移 / 删除），点击「配置」或「＋ 加参数」从右侧滑出抽屉面板集中配置——基本信息（显示名 / 参数ID / 类型 / 默认值 / 范围模式）、数据绑定（数据集 + 过滤字段，选字段自动回填 ID / 显示名）、手动下拉选项、候选值 SQL，区块按参数类型动态显隐；参数以 JS 模型数组为唯一数据源（输入实时提交，无脏数据），保存 / 试运行 / 字段枚举口径与旧版一致；数据集增删时已打开的面板自动刷新绑定选项

- **报表列表预览（只读发布视角）**：列表页每张报表新增「预览」入口，点击在新窗口打开独立的只读预览页（也可直接用完整 URL 访问）——页面仅包含报表名称 / 查询条件 / 查询 / 导出 / 数据区，无导航栏及任何编辑、新建、删除、管理入口，普通用户只能查看；新增轻量只读路由 `/pv/{id}`（与 `/r/{id}` 同套访问控制，token 模式需有效 `?t=`），复用完整查看页查询 / 导出链路

### 修复

- **查询结果取消千分位格式化（回归）**：表格数值单元格 `cellFmt` 与 KPI 摘要卡片移除 `toLocaleString('zh-CN', …)`——v0.3.0 曾修复、v0.4.0 数值列格式化时误回退；本次彻底移除，数字列保留右对齐 + `tabular-nums`，全部列按数据库返回值原样展示（如 `4261637631631253500` 不再显示为带千位分隔形式，避免大整数精度丢失）
